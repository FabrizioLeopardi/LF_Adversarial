...
def main():
    ...
    for line in range(len(NN_data2)):
        count+=1
        if (k==784 and l<10):
            bias_1[l] = float(NN_data2[line])
            l += 1
        if (k<784):
            weight_matrix_1[l][k] = float(NN_data2[line])
            l += 1
            if (l==10):
                l=0
                k += 1
    ...
    print(a)
    print("Handwritten digit recognised as a "+str(index_of_max(a[0])))
    
    if (a[0][2] == 0.0):
        print("The evasion attack worked!")
    else:
