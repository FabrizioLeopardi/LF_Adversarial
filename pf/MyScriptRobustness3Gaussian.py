"""
        ...
        python3 MyScriptRobustness3Gaussian.py '160x32' --batch-size 64 --epochs 10 --lr 0.01 --momentum 0.5 --seed 1 --save-model None --load-model ../Models/Frontnet160x32.pt --load-trainset '../Data/160x96OthersTrainsetAug.pickle' --load-testset '../Data/160x96StrangersTestset.pickle' --quantize --regime 'Scripts/regime.json'
...
def gaussian_additive_pixel(test_loader, std_dev):
    for i in range(len(test_loader.dataset.data)):
        for j in range(96):
            for k in range(160):
                test_loader.dataset.data[i][0][j][k] += random.gauss(0,std_dev)
                if (test_loader.dataset.data[i][0][j][k] < 0.0):
                    test_loader.dataset.data[i][0][j][k] = 0.0
                if (test_loader.dataset.data[i][0][j][k] > 255.0):
...
def main():
    ...
    for incr in range(tot_incr):
        test_loader = gaussian_additive_pixel(test_loader,std_dev)
        std_dev += 1
